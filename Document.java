...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
	    ...
	    for(int i=0; i<length; i++) {
	    	...
	    	if (i == (length - 1)) {
	    		...
	    		else if (isVowel && !previousIsVowel) {
	    			totalSyllables++;
	    		}
	    	}
	...
	private boolean isVowel(char charAt) {
		if (charAt == 'e' || charAt == 'i' || charAt == 'o' || charAt == 'u' || charAt == 'a' || charAt == 'y') {
			return true;
		}
		return false;
	}
	...
	public double getFleschScore()
	{
		double score = 206.835 - 
				(1.015 *
						((double)getNumWords()/getNumSentences())
				) - 
				(84.6 * 
						((double)getNumSyllables()/getNumWords())
				);
